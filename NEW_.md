...
**Model:** MCP server, 75 credits then a 48-hour cooldown, paid tier later
...
| 6 — Hosted HTTP + credit/cooldown policy | ✅ |
| 7 — Per-user accounts + per-user credit limits (HTTP edge) | ✅ |
...
- **Rate-limit tightening trigger:** only when daily requests exceed 5,000 for 2 consecutive weeks OR monthly infra+DB cost exceeds $15. Until then free tier = 75 credits followed by a 48-hour cooldown.
